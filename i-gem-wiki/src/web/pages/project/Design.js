...
function Design() {
  ...
  <div className="page" onMouseMove={onMove} onScroll={onScroll}>
      <CustomScrollbar>
        <BackgroundImage scroll={scroll} xy={xy} title="Design" src="https://2019.igem.org/wiki/images/5/5c/T--Potsdam--design.png"/>
        <div className="main-content">
          ...
          <a.div className="page-text red" style={propsInsilico}>
			...
			<p>In the beginning, none of us had any idea how to approach this problem as we come from an interdisciplinary background of biology and computer science. However, through careful planning, diligent research and endless communication of the needs and problems of all disciplines, we managed to tackle this notoriously hard problem and overcome many challenges. In the end, the results speak for themselves, which is why we feel confident in our project design and overall approach. </p>
			...
			<p>It has an input layer (the green one on the left), where we feed the network with data. Then it can have one or more hidden layers (the blue one) where most of the computation is going on, and finally an output layer (yellow one) were we get the results/predictions of our networks. </p>
			...
			<p>In our project, we went through five main iterations to continuously improve the results of our goal. First, we researched the current status in literature in relation to our project. Afterwards, we had two approaches with different datasets to improve our modeling to predict protein temperature stability. Building on the best models, we developed an approach to generate more heat resistance sequences. Lastly, we worked on further ideas which can be used to improve the results.</p>
			<h2>1. Literature research</h2>
			<p>Since we did not know anything about the literature in this problem domain, we had to familiarize ourselves with it from the ground up. During this process we identified the following problems in current published results: </p>
			...
			<p>From previous modeling approaches and results, we distilled that machine learning-based approaches, specifically relying on transfer learning, are likely to have the best effects on this topic.</p>
			...
			<p>This data was not sufficient for our use which is why we researched and aggregated data of other existing datasets.</p>
			...
			<p><b>BacDive [4]:</b></p>
			<p>This database contains various information on bacteria, including their growth temperature range. We used this database in combination with the NCBI database to construct our very own dataset, BacDive+. BacDive+ contains pairs of protein amino acid sequences and organism growth temperature information (of the organism the proteins sequence can be found in). The construction of BacDive+ is thus very similar to ProtDataTherm. However, we kept the real valued information about the organism growth temperature and did not convert it into thermal stability classes. The overall size of the dataset is about 7 Mio. sequences and 20k organisms. In the graph below you can see the distribution of the Averaged Organism Growth Temperature range (AOGTR) among the proteins:</p>
			...
			<p>As expected, mesophilic organisms are strongly over represented in BacDive+ but the total range of the dataset is still over 80°C.</p>
			<p><b>Merck&Co [1]:</b></p>
			...
			<p>From our literature research, we knew that support vector machines and random forests performed well on predicting dTm upon single point mutations. However, we wanted to advance this research with the following highly advanced and novel machine learning methods.</p>
			...
			<p>Cycle GANs are able to transform one image of domain A into another of domain B and back. Specifically, this was used for images in which a horse is used to look like a zebra. We wanted to use this to transform mesophilic proteins into thermophilic ones. The Problem is that changes made by the model are probably too drastic which then would also affect functionality and is a lot harder to validate.</p>
			<p><b>Deep Neural Networks:</b></p>
			<p>Neural networks have been tried by other researchers, but only very rudimentarily as they require a lot of data and for Deep Neural Networks with a lot of research. Two common types of neural networks are relevant for our use: Convolutional Neural Networks (CNN) as they perform well to learn more advanced feature about sequential data and residual layers which use jump connections to improve the learning process and avoid getting stuck on local minimas.</p>
			...
			<p>The input to the network was the label encoded sequence with the output being either 1 for thermostable, or 0 for mesostable. We evaluated the performance using the AUC value of the ROC curve, since ProtDataTherm is an unbalanced dataset (mesostable proteins are overrepresented).</p>
			...
			<p>Our next step was to more accurately determine the temperature stability of a protein. For this, we use the averaged  organism growth temperature range (AOGTR) of about 7 Mio. proteins found in the BacDive+ dataset.</p>
			...
			<p>Finally, we created an application that outputs mutants that it predicted to be more thermostable as well as very similar proteins found with tBlastn which are more heat resistant after being given an input amino acid sequence. For the latter, we ran our models on the most similar sequences of the tBlastn results and return the ones with most similarity and higher predicted heat resistance. Basically, we scan NCBI for similar proteins which might not have been fully characterized yet but we can use our Model to predict their stability.</p>
			<p>The mutation of the original sequence are done differently. We decided to use the tBlastn result to find mutations which naturally occur in proteins and only use these to be allowed for mutations which means their exact position and amino acid. To further avoid functional changes we don’t allow mutations in active centers which have to be previously set. </p>
			<p>Our experiments showed that we need to introduce multiple mutations to increase the melting temperature of our protein by a significant amount through a cumulative effect. We decided to use a simulated evolution approach as the search field for possible mutations is very large. The evolutionary approach works similar to evolution in nature including cross over of good mutations and a higher "survival" chance of mutations to be kept based on how much they improve thermostability. Put simply, mutations are added to sequence iteratively and only those mutations which improve the thermostability “survive”. Later on, survivors are crossed to find even better combinations of variants.</p>
			<p>We planned to use the model for the prediction of protein functions (for more information have a look at the <a href="https://2019.igem.org/Team:Potsdam/Improve">Improve</a> section) to also affect the survival rate of mutations to avoid those that change the functionality of the protein, but did not have the time to add this feature.</p>
			...
			<p>Lastly we started further improvements on the dataset as the data is the deciding factor in our modelling. Two main points we wanted to improve were that the dataset contained quite similar or duplicated proteins which we wanted to remove. What was additionally bad about them was that they often were from different organisms and had different temperature assigned which could hinder the training process of our model. </p>
			<p>To approach this issue, we used a computer cluster to compute identity and BLAST results of all sequence against all other which is are quite expensive operations for ~7Mio proteins. (<a href="https://github.com/soedinglab/mmseqs2/wiki#batch-sequence-searching-using-mmseqs-search">MMSEQS2 </a>) Afterwards we implemented an efficient clustering algorithm for our use case which is based of <a href="http://drive5.com/usearch/manual/uclust_algo.html">UClust </a>. It removes unnecessary proteins which are to similar and more correctly sets the highest temperature the cluster of these proteins is able to withstand. </p>
			<p>We also started research on more data we could provide the neural network with which could be easily calculated based on the primary structure. These were polarity, aromaticity, amino acid mass and further properties and we look forward to using the improved dataset in future work.</p>
