import React from 'react'
import './sponsors.css';

function Sponsors(props) {
  return (
    <div className="footer">
      <div className="img w170" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/c/c6/T--Potsdam--sponsor_0.png\')' }}></div>
      <div className="img w460" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/e/e5/T--Potsdam--sponsor_1.png\')' }}></div>
      <div className="img w200" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/b/b1/T--Potsdam--sponsor_2.png\')' }}></div>
      <div className="img w200" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/9/92/T--Potsdam--sponsor_3.png\')' }}></div>
      <div className="img w600" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/3/38/T--Potsdam--sponsor_5.png\')' }}></div>
      <div className="img" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/4/44/T--Potsdam--sponsor_4.png\')' }}></div>
      <div className="img w170" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/2/25/T--Potsdam--sponsor_6.jpg\')' }}></div>
      <div className="img w200" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/1/11/T--Potsdam--sponsor_7.png\')' }}></div>
      <div className="img w200" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/c/ce/T--Potsdam--sponsor_8.png\')' }}></div>
      <div className="img w-up" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/1/1f/T--Potsdam--sponsor_9.png\')' }}></div>
      <div className="img w224" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/e/ef/T--Potsdam--sponsor_10.png\')' }}></div>
      <div className="img w245" style={{backgroundImage:'url(\'https://2019.igem.org/wiki/images/e/e9/T--Potsdam--sponsor_11.png\')' }}></div>
    </div>
  )
}
export default Sponsors;
